...
- Accepts a function run again every element
- <code>fn(element, index, array)</code>
- Returns new array of same length with manipulated values
---
<CodeSurfer
...
<CodeSurfer
  ...
  steps={[
      ...
      { range: [7,10], notes: "With partial application, you can 'fix' the array before the value" },
      { range: [9,10], notes: "These scales are 'fixed' to the predicate" },
      { range: [12,15], notes: "Each resulting function 'remembers' the scale from the outer scope" },
  ]}
/>
---
...
<Wow id='preludeInC' title='Prelude In C'/>
---
