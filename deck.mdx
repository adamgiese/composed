...
These so-called "pure" functions are the building block.
```
...
```notes
- Humans have limited RAM called "working memory"
...
<CodeSurfer
  ...
  steps={[
      ...
      { range: [6,6], notes: "`freq` accepts a string ('A4') and returns a number (440)" },
      { range: [7,7], notes: "`isHigh` accepts a number and returns a boolean" },
...
<CodeSurfer
  ...
  steps={[
      ...
      { range: [10,10], notes: "Native partial application is currently in Stage 1" },
  ]}
/>
---
...
<CodeSurfer
  ...
  steps={[
      ...
      { range: [16,18], notes: "The pipeline operator (TC39 stage 1) would allow for easy function composition" },
  ]}
/>
...
<CodeSurfer
  ...
  steps={[
      ...
      { range: [33,33], notes: "It takes each of the preceding notes..."},
      ...
      { range: [40,47], notes: "`arrayMap` works like Array.map"},
